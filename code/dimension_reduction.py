...
def rsvd(X, dstar, power_iters=2):
    ...
    P = np.random.randn(X.shape[0],dstar)
...
def stabilityMeasure(X, d_max, B=5, power_iters=2):
    ...
    for i in range(B):
        U = rsvd(X,d_max, power_iters)[0] # MYI: add power iters
    ...
    p_vals = np.zeros(d_max-1) # MYI: I think there are actually d_max-1
    print(stability_vec)

    for k in range(1,d_max): # MYI: array indexing...
        p_vals[k-1] = stats.ranksums(stability_vec[0:k],stability_vec[k:d_max])[1]
  # MYI: k-1 -> k
    dstar = np.argmin(p_vals)+1
    print(p_vals)
    return dstar
